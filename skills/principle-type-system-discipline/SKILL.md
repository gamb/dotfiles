...
- **Store each fact once.** Two copies of one fact is a representable illegal state (the copies out of sync). If a cache or index is needed, put both copies behind one small module that owns the invariant.
