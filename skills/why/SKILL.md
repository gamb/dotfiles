...
Before spawning investigators, list the available MCP servers from the tools in this session (tool names of the form `mcp__<server>__<tool>`). Skip servers that report they need authentication.
...
- `subagent_type`: `"general-purpose"`. Do not use `Explore` here. It has no MCP access, which disables MCP-backed investigators entirely. Investigators still should not write anything.
...
- `subagent_type`: `"general-purpose"`. The synthesizer's quality check spot-verifies citations, which can require MCP access. `Explore` has no MCP access and defeats that.
