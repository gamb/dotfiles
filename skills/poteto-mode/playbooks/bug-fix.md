...
5. Stage the commits so the failing repro lands before the fix in git history. See the **poteto-tdd** skill for the failing-test-first cadence when the bug has a cheap local test path. Skip it when the test would be expensive, integration-heavy, or unclear.
